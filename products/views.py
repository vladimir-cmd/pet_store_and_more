...
def specific_products_dealz(request):
    ...
    categories = Category.objects.filter(
        name__in=['deals', 'clearance', 'new_items'])
    products = products.filter(
        category__name__in=['deals', 'clearance', 'new_items'])
...
def add_product(request):
    if not request.user.is_superuser:
        messages.error(
            request, "You don't have enough permissions to add product. Please contact administrator.")
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(
                request, 'Failed to update product. Please ensure that form is valid.')
    else:
